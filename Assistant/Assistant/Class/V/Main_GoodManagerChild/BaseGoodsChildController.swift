...
//    }
    ...
    func asyncWithParams(curIndex:Int) -> Void {
        ...
        self.goodViewModel?.resultArrayBlock = {(array:Array<Any>,isEnd:Bool) -> Void in
            if array.count == 0 {
                self.mj_footer.setTitle("暂无数据", for: MJRefreshState.idle)
                self.mj_footer.setTitle("暂无数据", for: MJRefreshState.willRefresh)
                self.mj_footer.setTitle("暂无数据", for: MJRefreshState.refreshing)
                self.mj_footer.setTitle("暂无数据", for: MJRefreshState.noMoreData)
