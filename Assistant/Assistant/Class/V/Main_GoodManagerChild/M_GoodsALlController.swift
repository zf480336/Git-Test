...
class M_GoodsALlController: BaseGoodsChildController {

    override func viewDidLoad() {
        super.viewDidLoad()
        self.Good_List = MyGoodList.Goods_All
        setViewModel()
    }
    ...
    override func setViewModel() -> Void {
        ...
        if self.goodViewModel?.dataArray.count == 0 {
            self.mj_footer.setTitle("请输入要查询的订单号", for: MJRefreshState.idle)
            self.mj_footer.setTitle("请输入要查询的订单号", for: MJRefreshState.willRefresh)
            self.mj_footer.setTitle("请输入要查询的订单号", for: MJRefreshState.refreshing)
            self.mj_footer.setTitle("请输入要查询的订单号", for: MJRefreshState.noMoreData)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (Int)(model.orderContainerNumber!)! < (Int)(model.container_number!)! {
            ...
            let detail_Controller = Good_Detail_HadAcceptController.init(style: UITableViewStyle.plain)
            detail_Controller.orderModel = model
            self.navigationController?.pushViewController(detail_Controller, animated: true)
        }
    }
